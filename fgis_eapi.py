...
def get_argv():
    ...
    parser.add_argument('-t', '--title', type=str, default='', help='Вид СИ')
    parser.add_argument('-y', '--year', type=int, default=0, help='Год последней поверки')
    parser.add_argument('-s', '--serial', type=str, default='', help='Серийный номер СИ')
...
def main():
    parser = get_argv()
    namespace_argv = parser.parse_args(sys.argv[1:])
    title = namespace_argv.title
    year = namespace_argv.year
    serial = namespace_argv.serial
    type = namespace_argv.type
    rows = namespace_argv.rows

    resp = request_fgis({'filter_mititle': title,
                         'verification_year': year,
                         'filter_minumber': serial,
                         'filter_mitype': type,
                         'rows': rows})
    print(resp)
